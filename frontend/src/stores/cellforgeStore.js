...
function poisson(lam) {
    if (lam <= 0)
        return 0;
    if (lam > 30)
        return Math.max(0, Math.round(lam + Math.sqrt(lam) * (Math.random() * 2 - 1)));
...
let _interval = null;
export const useCellForgeStore = create((set, get) => ({
    state: initState(),
    history: [],
    isRunning: false,
    speed: 1,
