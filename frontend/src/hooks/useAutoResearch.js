...
export function useResearchManager() {
    const { genome, isAnalyzing, handleEvent, geneAnalysisStatus } = useGeneStore();
    ...
    const [batchCurrentGene, setBatchCurrentGene] = useState(null);
    ...
    const startBatchResearch = useCallback(() => {
        if (batchActive || queue.length === 0 || !genome)
            return;
    ...
    useEffect(() => {
        if (!batchActive || !loopActiveRef.current || !genome)
            return;
        // Don't start a new analysis while one is running
        if (geneAnalysisStatus === "running")
            return;
        ...
        function launchNext(i) {
            const item = batchQueue[i];
            if (!item)
                return;
            const genomeGene = genome.genes.find((g) => g.locus_tag === item.locus_tag);
    ...
    const researchSingleGene = useCallback((locusTag) => {
        if (batchActive || !genome)
            return;
        const genomeGene = genome.genes.find((g) => g.locus_tag === locusTag);
        if (!genomeGene || !genomeGene.protein_sequence)
            return;
        analyzeGene(genomeGene);
