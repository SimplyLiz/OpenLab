...
export function useCellForgeSimulation() {
    ...
    const createSimulation = useCallback(async (organismName) => {
    ...
    const startSimulation = useCallback(async (id) => {
        const res = await fetch(`${API_BASE}/${id}/start`, { method: 'POST' });
        return res.json();
    }, []);
    const stopSimulation = useCallback(async (id) => {
        const res = await fetch(`${API_BASE}/${id}/stop`, { method: 'POST' });
        return res.json();
    }, []);
    const injectPerturbation = useCallback(async (id, perturbation) => {
