...
export function CellSimulation({ compact = false }) {
    const canvasRef = useRef(null);
    const animRef = useRef(0);
    const frameRef = useRef(0);
    // Idle mode refs
    const idleParticlesRef = useRef([]);
    // Live mode refs
    ...
    function drawIdleMode(ctx, cx, cy, _w, h, t, frame) {
        ...
        for (let layer = 0; layer < 2; layer++) {
            ...
            for (let a = 0; a < Math.PI * 2; a += 0.02) {
                const wobble = Math.sin(a * 8 + t * 0.5) * 2 + Math.sin(a * 3 - t * 0.3) * 1.5;
    ...
    function drawLiveMode(ctx, cx, cy, _w, h, t) {
        ...
        ctx.fillText(`t = ${timeHours}h    vol = ${volStr} fL    divisions = ${divStr}`, cx, infoY);
    ...
    return (_jsxs("div", { className: "cell-sim-wrapper", children: [_jsx("canvas", { ref: canvasRef, className: "cell-sim-canvas", style: { width: "100%", height: compact ? "150px" : "400px" } }), isComplete && simulationSnapshots.length > 1 && (_jsxs("div", { className: "cell-sim-controls", children: [_jsx("input", { type: "range", className: "cell-sim-slider", min: 0, max: simulationSnapshots.length - 1, value: playbackIndex >= 0 ? playbackIndex : simulationSnapshots.length - 1, onChange: (e) => {
                            setIsPlaying(false);
                            setPlaybackIndex(Number(e.target.value));
                        } }), _jsxs("div", { className: "cell-sim-controls-row", children: [_jsx("button", { className: "cell-sim-btn", onClick: () => {
                                    if (!isPlaying && playbackIndex >= simulationSnapshots.length - 1) {
                                        setPlaybackIndex(0);
                                    }
                                    setIsPlaying(!isPlaying);
                                }, children: isPlaying ? "Pause" : "Play" }), _jsxs("label", { className: "cell-sim-speed", children: ["Speed", _jsxs("select", { value: playbackSpeed, onChange: (e) => setPlaybackSpeed(Number(e.target.value)), children: [_jsx("option", { value: 10, children: "10 fps" }), _jsx("option", { value: 30, children: "30 fps" }), _jsx("option", { value: 60, children: "60 fps" }), _jsx("option", { value: 120, children: "120 fps" })] })] }), _jsxs("span", { className: "cell-sim-frame-info", children: [playbackIndex >= 0 ? playbackIndex + 1 : simulationSnapshots.length, " / ", simulationSnapshots.length] })] })] }))] }));
}
