...
`;
export function CellRenderer({ position }) {
    const groupRef = useRef(null);
    const membraneRef = useRef(null);
    const ribosomeRef = useRef(null);
    const mrnaRef = useRef(null);
    const nucleoidRef = useRef(null);
    const glowRef = useRef(null);
    const flagellaRef = useRef(null);
    ...
    const membraneColor = useMemo(() => {
        if (stressed)
            return new THREE.Color(0.9, 0.25, 0.15);
    ...
    useFrame(() => {
        ...
        if (membraneRef.current) {
            const mat = membraneRef.current.material;
            ...
            membraneRef.current.scale.set(scale * elongation * breath, scale * 0.82 * pinch * breath, scale * breath);
        }
        // Ribosomes
        if (ribosomeRef.current) {
            for (let i = 0; i < RIBOSOME_COUNT; i++) {
                ...
                _dummy.position.set(bx + Math.sin(t * 2.2 + i * 1.1) * j, by + Math.cos(t * 2.8 + i * 0.7) * j, bz + Math.sin(t * 1.6 + i * 1.5) * j);
        ...
        if (mrnaRef.current) {
            ...
            for (let i = 0; i < MRNA_COUNT; i++) {
                ...
                _dummy.position.set(bx + m.dir[0] * drift, by + Math.cos(t * 1.0 + i) * 0.025, bz + m.dir[2] * drift);
                ...
                _dummy.rotation.set(Math.sin(t * 0.7 + i) * 0.3, 0, -Math.PI / 2 + angle + Math.cos(t * 0.5 + i) * 0.3);
                _dummy.updateMatrix();
        ...
        if (flagellaRef.current) {
            const geom = flagellaRef.current.geometry;
            const posArr = geom.attributes.position;
            if (posArr) {
                const orig = geom._origPositions;
                if (!orig) {
                    geom._origPositions = Float32Array.from(posArr.array);
                }
                else {
    ...
    return (_jsxs("group", { ref: groupRef, position: position, children: [_jsxs("mesh", { ref: membraneRef, renderOrder: 10, children: [_jsx("sphereGeometry", { args: [1, 64, 48] }), _jsx("shaderMaterial", { vertexShader: membraneVert, fragmentShader: membraneFrag, uniforms: membraneUniforms, transparent: true, side: THREE.FrontSide, depthWrite: false })] }), _jsxs("mesh", { ref: nucleoidRef, renderOrder: 1, children: [_jsx("icosahedronGeometry", { args: [1, 2] }), _jsx("meshStandardMaterial", { color: "#0a1540", emissive: "#122266", emissiveIntensity: 0.25 + state.replicationProgress * 0.3, roughness: 0.95, metalness: 0 })] }), _jsxs("mesh", { ref: glowRef, renderOrder: 0, children: [_jsx("icosahedronGeometry", { args: [1, 1] }), _jsx("meshBasicMaterial", { color: "#1a2266", transparent: true, opacity: 0.04, blending: THREE.AdditiveBlending, depthWrite: false })] }), state.replicationProgress > 0.01 && (_jsxs("mesh", { rotation: [Math.PI / 2, 0, 0], scale: [0.32 * scale, 0.32 * scale, 0.015], renderOrder: 2, children: [_jsx("torusGeometry", { args: [1, 0.1, 6, 48, Math.PI * 2 * state.replicationProgress] }), _jsx("meshStandardMaterial", { color: "#1a2244", emissive: "#334488", emissiveIntensity: 0.3 })] })), _jsxs("instancedMesh", { ref: ribosomeRef, args: [undefined, undefined, RIBOSOME_COUNT], renderOrder: 3, children: [_jsx("icosahedronGeometry", { args: [1, 0] }), _jsx("meshStandardMaterial", { color: "#443010", emissive: "#886620", emissiveIntensity: 0.2 + growthNorm * 0.15, roughness: 0.9 })] }), _jsxs("instancedMesh", { ref: mrnaRef, args: [undefined, undefined, MRNA_COUNT], renderOrder: 4, children: [_jsx("cylinderGeometry", { args: [1, 0.4, 1, 3] }), _jsx("meshStandardMaterial", { color: "#0a2a15", emissive: "#186630", emissiveIntensity: 0.2, roughness: 0.9 })] }), _jsx("mesh", { ref: flagellaRef, geometry: flagellaGeom, renderOrder: 5, children: _jsx("meshStandardMaterial", { color: "#2a3844", emissive: "#334455", emissiveIntensity: 0.15, roughness: 0.8 }) })] }));
}
