...
export function MetaboliteParticles() {
    ...
    useFrame(() => {
        ...
        for (let i = 0; i < COUNT; i++) {
            ...
            p.vx[i] -= p.x[i] * 0.001;
            p.vz[i] -= p.z[i] * 0.001;
            if (Math.abs(p.x[i]) > BOUNDS)
                p.vx[i] -= Math.sign(p.x[i]) * 0.006;
            if (Math.abs(p.z[i]) > BOUNDS)
                p.vz[i] -= Math.sign(p.z[i]) * 0.006;
    ...
    return (_jsxs("points", { ref: pointsRef, frustumCulled: false, children: [_jsxs("bufferGeometry", { children: [_jsx("bufferAttribute", { attach: "attributes-position", args: [positions, 3] }), _jsx("bufferAttribute", { attach: "attributes-color", args: [colors, 3] }), _jsx("bufferAttribute", { attach: "attributes-size", args: [sizes, 1] })] }), _jsx("shaderMaterial", { vertexShader: pointsVert, fragmentShader: pointsFrag, uniforms: uniforms, vertexColors: true, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending })] }));
}
