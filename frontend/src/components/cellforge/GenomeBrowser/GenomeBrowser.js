...
export function GenomeBrowser() {
    ...
    return (_jsxs("div", { style: { padding: 8, height: '100%', display: 'flex', flexDirection: 'column' }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 12 }, children: "Genome Browser" }), _jsxs("span", { style: { fontSize: 10, color: '#888' }, children: [TOTAL_LEN.toLocaleString(), " bp"] })] }), _jsxs("svg", { width: "100%", height: TRACK_H + 20, viewBox: `0 0 ${W} ${TRACK_H + 20}`, style: { minWidth: 400 }, children: [_jsx("line", { x1: 0, y1: TRACK_H / 2, x2: W, y2: TRACK_H / 2, stroke: "#444", strokeWidth: 2 }), GENES.map((gene) => {
                        ...
                        return (_jsxs("g", { onClick: () => setSelected(isSel ? null : gene.id), style: { cursor: 'pointer' }, children: [_jsx("rect", { x: x, y: 2, width: w, height: TRACK_H - 4, rx: 3, fill: color, stroke: isSel ? '#fff' : 'none', strokeWidth: isSel ? 1.5 : 0 }), _jsx("text", { x: x + w / 2, y: TRACK_H + 14, textAnchor: "middle", fill: "#aaa", fontSize: 8, children: gene.id })] }, gene.id));
                    })] }), selected && (_jsx(GeneDetail, { geneId: selected, product: GENES.find((g) => g.id === selected)?.product ?? '', mrna: state.mrnaCounts[selected] ?? 0, protein: state.proteinCounts[selected] ?? 0, isKO: knockedOut.has(selected) }))] }));
}
