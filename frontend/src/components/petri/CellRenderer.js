/**
 * CellRenderer — Pure canvas drawing functions for single-cell visualization.
 * Zero React dependencies. Extracted from CellSimulation.js for reuse.
 */
...
export function generateLiveParticles(snapshot, rx, ry) {
...
export function drawIdleCell(ctx, cx, cy, w, h, t, frame, genome, idleParticles) {
...
export function drawMembrane(ctx, cx, cy, rx, ry, t, flashAlpha) {
    ...
    // Membrane line with subtle wobble
    ctx.save();
    ctx.beginPath();
    for (let a = 0; a <= Math.PI * 2; a += 0.02) {
...
export function drawChromosomeRing(ctx, cx, cy, rx, ry) {
    ctx.save();
    ctx.beginPath();
...
export function drawParticles(ctx, cx, cy, particles) {
...
export function drawInfoOverlay(ctx, cx, h, snapshot) {
    const infoY = h - 50;
    ctx.fillStyle = "#94a3b8";
    ctx.font = "10px JetBrains Mono, monospace";
    ctx.textAlign = "center";
...
export function drawLegend(ctx, cx, h) {
    const legendY = h - 18;
...
export function drawLiveCell(ctx, cx, cy, rx, ry, t, h, snapshot, liveParticles, flashAlpha) {
    drawMembrane(ctx, cx, cy, rx, ry, t, flashAlpha);
    drawChromosomeRing(ctx, cx, cy, rx, ry);
    drawParticles(ctx, cx, cy, liveParticles);
    drawInfoOverlay(ctx, cx, h, snapshot);
    drawLegend(ctx, cx, h);
}
