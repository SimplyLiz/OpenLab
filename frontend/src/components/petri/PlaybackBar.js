...
export function PlaybackBar() {
    ...
    // Playback timer
    useEffect(() => {
        if (!isComplete || !isPlaying || total === 0)
            return;
    ...
    if (!isComplete || total < 2)
        return null;
    const currentFrame = playbackIndex >= 0 ? playbackIndex : total - 1;
    return (_jsxs("div", { className: "glass-panel playback-bar", children: [_jsx("button", { className: "playback-btn", onClick: () => {
                    if (!isPlaying && playbackIndex >= total - 1) {
                        setPlaybackIndex(0);
                    }
                    setIsPlaying(!isPlaying);
                }, children: isPlaying ? "\u23F8" : "\u25B6" }), _jsx("input", { type: "range", className: "playback-slider", min: 0, max: total - 1, value: currentFrame, onChange: (e) => {
                    setIsPlaying(false);
                    setPlaybackIndex(Number(e.target.value));
                } }), _jsxs("span", { className: "playback-frame", children: [currentFrame + 1, " / ", total] }), _jsxs("select", { className: "playback-speed", value: speed, onChange: (e) => setSpeed(Number(e.target.value)), children: [_jsx("option", { value: 10, children: "10fps" }), _jsx("option", { value: 30, children: "30fps" }), _jsx("option", { value: 60, children: "60fps" }), _jsx("option", { value: 120, children: "120fps" })] })] }));
}
